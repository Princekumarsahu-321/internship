...
const googleLogin = async (req, res) => {
  try {
    ...
    if (decoded.aud && !audience.includes(googleClientId)) {
    ...
    if (!user) {
      ...
      user = await User.create({
        ...
        phoneNumber: `google-${Date.now()}-${crypto.randomBytes(6).toString("hex")}`,
        avatar: {
          url: decoded.picture || defaultAvatarUrl(name),
        },
        googleId,
        authProvider: "google",
      });
      ...
      if (!user.avatar || !user.avatar.url) {
        user.avatar = { url: decoded.picture || defaultAvatarUrl(name) };
      }
    ...
    if (sentPhoto) {
      ...
      filteredBody.avatar = {
        url: defaultAvatarUrl(
          filteredBody.name || req.user.name || req.user.username
        ),
      };
    } else {
